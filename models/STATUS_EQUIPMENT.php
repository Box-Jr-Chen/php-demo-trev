    ...
    class STATUS{
        ...
        public function create_update(){
         ...
         // INSERT INTO '.$this->table.' (equipid,brand,devip) VALUES (key, brand_c,devip_c,subid_c,opid_c,status_c,mode_c,linkstatus_c,palletno_c,mission_c,palletsize_c);
                    $query = 'CREATE OR REPLACE FUNCTION merge_db(key varchar,brand_c varchar,devip_c varchar,subid_c varchar,opid_c varchar,status_c varchar,mode_c varchar,linkstatus_c varchar,palletno_c varchar,mission_c varchar,palletsize_c varchar,palletsize2_c varchar) RETURNS VOID AS
            $$
            BEGIN
                    UPDATE '.$this->table.' SET brand=brand_c,devip=devip_c,subid=subid_c,opid=opid_c,status=status_c,mode=mode_c,linkstatus=linkstatus_c,palletno=palletno_c,mission=mission_c,palletsize=palletsize_c  WHERE equipid = key;
                    IF found THEN
                    ...
                    BEGIN
                        INSERT INTO '.$this->table.' (equipid,brand,devip,subid,opid,status,mode,linkstatus,palletno,mission,palletsize) VALUES (key, brand_c,devip_c,subid_c,opid_c,status_c,mode_c,linkstatus_c,palletno_c,palletsize_c,mission_c);
                        RETURN;
          ...
          //  echo ($query);
             ...
             if($stmt->execute()){
                $query2 = 'SELECT merge_db(:equipid,:brand,:devip,:subid,:opid,:status_c,:mode,:linkstatus,:palletno,:mission,:palletsize,:palletsize);';
